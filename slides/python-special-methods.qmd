...
format: 
    revealjs:
        theme: default   # other themes: beige, blood, dark, dracula, league, moon, night, serif, simple, sky, solarized
        slide-number: true
...
```{python}
x = 1; 
x.__add__(2) # this one is also ok
```
...
## Comparison operator methods: Example
...
class Complex:
    ...
    def __ne__(self, other):
        if isinstance(other, type(self)):
            return self.real != other.real or self.image != other.imag
```
...
- There are a lot of special methods that cannot be studied through this course.
- Complete reference can be found from [this official Python documentation](https://docs.python.org/3.13/reference/datamodel.html#special-method-names).
